class MemoryTrainer{constructor(){this.questions=[],this.allQuestions=[],this.sharedOptions=[],this.trainingQueue=[],this.wrongQuestions=[],this.currentIndex=0,this.isActive=!1,this.isReviewMode=!1,this.isFromList=!1,this.examType="hoeren",this.currentCorrectText="",this.currentCorrectIndex=-1,this.currentOptions=[],this.currentQuestionIndex=0,this.currentExamId=1,this.currentQuestionObj=null,this.attempts=0,this.correctAttempts=0,this.totalQuestions=0,this.overlay=null,this.card=null,this.timer=null,this.isAnswered=!1,this.isCardReady=!1,this.TOTAL_OPTIONS=3,this.WRONG_OPTIONS=2,this.LEVELS_KEY="memory_levels",this.MAX_LEVEL=5,this.currentSkill="hoeren1",this.currentExamId=1,this.examSharedOptionsMap={}}async getUserStatus(){try{return typeof window.getUserStatusGlobal=="function"?await window.getUserStatusGlobal():typeof window.getUserStatusForExam=="function"?await window.getUserStatusForExam():"free"}catch(e){return console.warn("⚠️ فشل جلب حالة المستخدم:",e),"free"}}async start(e="single"){console.log(`🧠 بدء Memory Trainer V4 (المهارة: ${this.currentSkill}, الوضع: ${e})...`);let t=null;if(this.isFromList=!1,this.sharedOptions=[],this.examSharedOptionsMap={},e==="list"){const i=`_${this.currentSkill}_combinedData`;if(window[i]){if(t=window[i],this.isFromList=!0,console.log(`📚 تدريب من قائمة ${this.currentSkill} (المرحلة ${t.currentStage||1})`),this.examType=t.examType||"hoeren",this.currentSkill==="lesen1"||this.currentSkill==="lesen3"?this.examType="matching":this.currentSkill==="lesen2"?this.examType="multiple":this.currentSkill==="sprach1"?this.examType="sprach1":this.currentSkill==="sprach2"&&(this.examType="sprach2"),this.currentSkill==="lesen1"||this.currentSkill==="lesen3"){const n=t.examIds||[];if(n.length===0){this.showNotAvailable("لا توجد امتحانات في هذه المرحلة");return}let s=[];for(const o of n)try{const r=await fetch(`data/${this.currentSkill}/exam${o}.json`);if(!r.ok){console.warn(`⚠️ لا يمكن تحميل الامتحان ${o}`);continue}const a=await r.json();let l=[];if(this.currentSkill==="lesen1"&&a.sharedOptions?l=a.sharedOptions:this.currentSkill==="lesen3"&&a.situations&&(l=a.situations),l.length===0){console.warn(`⚠️ الامتحان ${o} ليس لديه sharedOptions، سيتم تخطيه`);continue}this.examSharedOptionsMap[o]=l;let p=[];this.currentSkill==="lesen1"?p=a.questions||[]:this.currentSkill==="lesen3"&&(p=a.items||[],p=p.filter(d=>d.correct!==null&&d.correct!==void 0&&typeof d.correct=="number"&&d.correct>=0&&d.correct<l.length));for(const d of p){const c={...d,examId:o,_sharedOptions:l};s.push(c)}console.log(`✅ تم تحميل الامتحان ${o} (${p.length} سؤال)`)}catch(r){console.warn(`⚠️ فشل تحميل الامتحان ${o}:`,r)}if(s.length===0){this.showNotAvailable("لا توجد أسئلة صالحة للتدريب في هذه المرحلة");return}if(this.questions=this.shuffleArray(s),this.allQuestions=this.questions.slice(),console.log(`📊 تم جمع ${this.questions.length} سؤال من ${n.length} امتحان`),n.length>0){const o=n[0];this.examSharedOptionsMap[o]&&(this.sharedOptions=this.examSharedOptionsMap[o])}}else{let n=t.allQuestions||[];this.currentSkill==="lesen1"||this.currentSkill==="lesen2"||this.currentSkill==="lesen3"||this.currentSkill==="sprach1"||this.currentSkill==="sprach2"?this.questions=n:this.questions=n.filter(s=>s.correct===!0),this.allQuestions=n,t.sharedOptions&&(this.sharedOptions=t.sharedOptions)}if(this.sharedOptions||(this.sharedOptions=[]),this.questions.length===0){this.showNotAvailable("لا توجد إجابات صحيحة في هذا الامتحان");return}}else if(typeof window.loadStageExams=="function"){await window.loadStageExams(this.currentSkill),window[i]?this.start(e):this.showNotAvailable(`لم يتم تحميل بيانات ${this.currentSkill} بعد`);return}else{this.showNotAvailable(`لم يتم تحميل بيانات ${this.currentSkill} بعد`);return}}else if(t=window.currentExamData||window._currentExamData,t){this.currentSkill=window.currentSkill||"hoeren1",this.currentExamId=window.currentExamId||1,console.log(`📖 تدريب من امتحان فردي: ${this.currentSkill} exam${this.currentExamId}`),this.currentSkill==="lesen3"&&t.situations&&!this.sharedOptions.length?this.sharedOptions=t.situations:t.sharedOptions&&(this.sharedOptions=t.sharedOptions),this.examType=t.type||"hoeren",this.currentSkill==="lesen1"||this.currentSkill==="lesen3"?this.examType="matching":this.currentSkill==="lesen2"?this.examType="multiple":this.currentSkill==="sprach1"?this.examType="sprach1":this.currentSkill==="sprach2"&&(this.examType="sprach2");let i=[];if(this.currentSkill==="sprach1"||this.currentSkill==="sprach2"?i=(t.options&&Array.isArray(t.options)?t.options:t.questions||[]).filter(s=>s.memoryHighlight).map((s,o)=>{const r=s.memoryHighlight||{};return{text:s.text||"",correct:s.correct,options:s.options||[],examId:this.currentExamId,questionIndex:o,originalQuestion:s,memoryHighlight:s.memoryHighlight||null,id:s.id,before:r.before||"",connector:r.connector||"",after:r.after||"",color:0,_sharedOptions:this.sharedOptions}}):this.currentSkill==="lesen3"&&t.items?(i=(t.items||[]).map((s,o)=>({...s,examId:this.currentExamId,questionIndex:o,_sharedOptions:this.sharedOptions})),this.sharedOptions.length>0&&(i=i.filter(s=>s.correct!==null&&s.correct!==void 0&&typeof s.correct=="number"&&s.correct>=0&&s.correct<this.sharedOptions.length))):i=(t.questions||[]).map((s,o)=>({...s,examId:this.currentExamId,questionIndex:o,_sharedOptions:this.sharedOptions})),this.questions=i,this.allQuestions=i.slice(),this.questions.length===0){this.showNotAvailable("لا توجد أسئلة صالحة في هذا الامتحان");return}}else{this.showNotAvailable("لا توجد بيانات امتحان");return}if(this.currentSkill==="lesen3"&&this.sharedOptions.length>0){const i=this.questions.length;this.questions=this.questions.filter(s=>s.correct!==null&&s.correct!==void 0&&typeof s.correct=="number"&&s.correct>=0&&s.correct<this.sharedOptions.length);const n=this.questions.length;n<i&&console.log(`🔍 Lesen 3: تم استبعاد ${i-n} فقرة غير صالحة (correct == null أو خارج النطاق)، بقي ${n} فقرة للتدريب`)}if(this.questions.length===0){this.showNotAvailable("لا توجد إجابات صحيحة في هذا الامتحان");return}if(this.buildTrainingQueue(),this.trainingQueue.length===0){this.showNotAvailable("لا توجد جمل للتدريب");return}this.isActive=!0,this.isReviewMode=!1,this.currentIndex=0,this.attempts=0,this.correctAttempts=0,this.wrongQuestions=[],this.totalQuestions=this.trainingQueue.length,this.isCardReady=!1,this.createOverlay(),this.createCardStructure(),this.isFromList?this.showIntroCardList():this.showIntroCardSingle()}createOverlay(){this.overlay&&this.overlay.remove(),this.overlay=document.createElement("div"),this.overlay.className="memory-trainer-overlay",this.overlay.addEventListener("click",e=>{if(e.target===this.overlay){if(this.currentIndex>=this.trainingQueue.length&&this.isActive){this.wrongQuestions.length>0?this.showPhaseComplete():this.showResults();return}this.close()}}),document.body.appendChild(this.overlay)}createCardStructure(){this.overlay||(console.warn("⚠️ createCardStructure: overlay غير موجود، يتم إنشاؤه تلقائياً"),this.createOverlay());const e=this.overlay.querySelector(".memory-trainer-card-container");e&&e.remove(),this.card=document.createElement("div"),this.card.className="memory-trainer-card-container",this.card.style.cssText=`
            width: 100%;
            display: flex;
            align-items: center;
            justify-content: center;
            animation: memorySlideUp 0.15s ease;
        `,this.overlay.appendChild(this.card),this.isCardReady=!0}updateCard(e){(!this.isCardReady||!this.card)&&this.createCardStructure(),this.card.innerHTML=e}buildTrainingQueue(){const e=this.questions.map(s=>s),t=Math.ceil(e.length/2),i=this.shuffleArray([...e]),n=[];for(let s=0;s<Math.min(t,i.length);s++)n.push(i[s]);this.trainingQueue=this.shuffleArray([...e,...n]),console.log(`📊 قائمة التدريب: ${this.trainingQueue.length} جملة (${this.isFromList?"مرحلة":"امتحان فردي"})`)}buildSentenceId(e,t,i){return window.buildSentenceId?window.buildSentenceId(e,t,i):`${e}_exam${t}_${i}`}getSentenceLevel(e){const t=JSON.parse(localStorage.getItem(this.LEVELS_KEY)||"{}");return t[e]!==void 0?t[e]:0}setSentenceLevel(e,t){const i=JSON.parse(localStorage.getItem(this.LEVELS_KEY)||"{}");let n=Math.max(0,Math.min(this.MAX_LEVEL,t));i[e]=n,localStorage.setItem(this.LEVELS_KEY,JSON.stringify(i))}increaseLevel(e){const t=this.getSentenceLevel(e);if(t<this.MAX_LEVEL){const i=t+1;this.setSentenceLevel(e,i),console.log(`⬆️ زيادة مستوى ${e} -> ${i}`)}}decreaseLevel(e){const t=this.getSentenceLevel(e);if(t>0){const i=t-1;this.setSentenceLevel(e,i),console.log(`⬇️ إنقاص مستوى ${e} -> ${i}`)}}getExamProgress(e,t){if(window.getExamProgress)return window.getExamProgress(e,t);const i=`${e}_exam${t}_`,n=JSON.parse(localStorage.getItem(this.LEVELS_KEY)||"{}");let s=0,o=0;for(const r in n)r.startsWith(i)&&(s+=n[r],o++);return o===0?0:Math.min(100,Math.round(s/(o*this.MAX_LEVEL)*100))}getOverallProgressForSkill(e){return window.getOverallProgress?window.getOverallProgress.length===1?window.getOverallProgress(e):window.getOverallProgress():0}getStageProgressForSkill(e){return window.getStageProgress?window.getStageProgress(e):0}generateOptions(e,t){const i=[e];let n=0;if(this.examType==="matching"){let r=null;if(t&&t._sharedOptions&&t._sharedOptions.length>0?r=t._sharedOptions:t&&t.examId&&this.examSharedOptionsMap[t.examId]?r=this.examSharedOptionsMap[t.examId]:this.sharedOptions&&this.sharedOptions.length>0&&(r=this.sharedOptions),r&&r.length>0){const a=t.correct,l=r[a],p=r.filter((x,g)=>g!==a),c=this.shuffleArray([...p]).slice(0,2),h=[l,...c];for(;h.length<3;){const x=r[Math.floor(Math.random()*r.length)];h.includes(x)||h.push(x)}return this.shuffleArray(h)}}const s=this.allQuestions.filter(r=>r.text!==e).map(r=>r.text);let o=this.shuffleArray([...s]);for(let r=0;r<o.length&&n<this.WRONG_OPTIONS;r++){const a=o[r];!i.includes(a)&&a.trim()!==""&&(i.push(a),n++)}for(;i.length<this.TOTAL_OPTIONS;)console.warn("⚠️ لم يتم العثور على جمل خاطئة كافية، نضيف جملة وهمية مؤقتة"),i.push(`جملة ${i.length+1}`);return this.shuffleArray(i)}showIntroCardSingle(){const e=this.getExamProgress(this.currentSkill,this.currentExamId);let t=`امتحان ${this.currentExamId}`;this.examType==="matching"?this.currentSkill==="lesen3"?t=`امتحان ${this.currentExamId} (Lesen 3)`:t=`امتحان ${this.currentExamId} (Lesen 1)`:this.examType==="multiple"?t=`امتحان ${this.currentExamId} (Lesen 2)`:this.examType==="sprach1"?t=`امتحان ${this.currentExamId} (Sprachbausteine 1)`:this.examType==="sprach2"&&(t=`امتحان ${this.currentExamId} (Sprachbausteine 2)`),this.updateCard(`
        ...
        `)}showIntroCardList(){this.getUserStatus().then(e=>{const t=e==="premium",i=this.getOverallProgressForSkill(this.currentSkill),n=this.trainingQueue.length;let s=1,o=1;window.getCurrentStage&&window.getTotalStages&&(s=window.getCurrentStage(this.currentSkill),o=window.getTotalStages(this.currentSkill));let r=this.currentSkill;this.examType==="matching"?this.currentSkill==="lesen3"?r="Lesen 3":r="Lesen 1":this.examType==="multiple"?r="Lesen 2":this.examType==="sprach1"?r="Sprachbausteine 1":this.examType==="sprach2"&&(r="Sprachbausteine 2");let a="";t?a='<button class="memory-trainer-btn primary" onclick="window.memoryTrainer.showMemoryCard()">ابدأ التدريب</button>':a=`
                ...
                <div class="memory-trainer-intro">
                    ...
                    <p style="font-size:12px;color:#94A3B8;margin:4px 0 4px 0;">${n} نص للتدريب</p>
            ...
            `)})}showMemoryCard(){if(this.clearTimer(),this.isAnswered=!1,this.currentIndex>=this.trainingQueue.length){this.showPhaseComplete();return}const e=this.trainingQueue[this.currentIndex],t=e.text;this.currentCorrectText=t,this.currentExamId=e.examId||this.currentExamId,this.currentQuestionIndex=e.questionIndex||0,this.currentQuestionObj=e,this.currentCorrectIndex=e.correct;let i=null;e._sharedOptions&&e._sharedOptions.length>0?i=e._sharedOptions:e.examId&&this.examSharedOptionsMap[e.examId]?i=this.examSharedOptionsMap[e.examId]:this.sharedOptions&&this.sharedOptions.length>0&&(i=this.sharedOptions);let n="";if(this.examType==="matching"){const s=this.currentCorrectIndex,o=i&&i[s]||"",r=o.match(/^[a-zA-Z][\.\)]\s*/)?o.match(/^[a-zA-Z][\.\)]\s*/)[0]:"",a=o.replace(/^[a-zA-Z][\.\)]\s*/,"");n=`
            ...
            `}else if(this.examType==="multiple"){const s=this.currentQuestionIndex!==void 0?this.currentQuestionIndex+1:this.currentIndex+1;let r=e.options&&e.options.length>0?e.options[e.correct]:"";/^[a-zA-Z][\.\)]\s*/.test(r)&&(r=r.replace(/^[a-zA-Z][\.\)]\s*/,"")),n=`
                <div class="memory-trainer-card" style="
                    background: #FFFFFF;
                    border: 1px solid #E8EEF5;
                    border-radius: 12px;
...
">
    ✅ ${r}
            ...
            `}else if(this.examType==="sprach1"){const s=e.memoryHighlight||{},o=e.id||this.currentQuestionIndex+1,r=s.before||"",a=s.connector||"",l=s.after||"";n=`
                <div class="memory-trainer-card" style="
                    background: #FFFFFF;
                    border: 1px solid #E8EEF5;
                    border-radius: 12px;
...
<div class="memory-reading-box" style="
    ...
    margin: 8px 0 12px 0;
">
    ${r} <span style="font-weight:700;color:#1565C0;background:#E3F2FD;padding:0 6px;border-radius:4px;">[${o}]</span> ${l}
</div>
...
">
    ✅ ${a}
            ...
            `}else if(this.examType==="sprach2"){const s=e.memoryHighlight||{},o=e.id||this.currentQuestionIndex+1,r=s.before||"",a=s.connector||"",l=s.after||"";n=`
                <div class="memory-trainer-card" style="
                    background: #FFFFFF;
                    border: 1px solid #E8EEF5;
                    border-radius: 12px;
            ...
            `;this.updateCard(n)}readyToRecall(){this.clearTimer();const e=this.currentQuestionObj;let t=null;if(e._sharedOptions&&e._sharedOptions.length>0?t=e._sharedOptions:e.examId&&this.examSharedOptionsMap[e.examId]?t=this.examSharedOptionsMap[e.examId]:this.sharedOptions&&this.sharedOptions.length>0&&(t=this.sharedOptions),this.examType==="matching"){const o=this.currentCorrectIndex,r=t[o],a=t.filter((l,p)=>p!==o).sort(()=>Math.random()-.5).slice(0,2);for(;a.length<2;){const l=t[Math.floor(Math.random()*t.length)];!a.includes(l)&&l!==r&&a.push(l)}this.currentOptions=this.shuffleArray([r,...a])}else if(this.examType==="multiple")this.currentOptions=e.options||[],this.currentOptions.length===0&&(console.warn("⚠️ لا توجد خيارات في السؤال، نستخدم generateOptions كحل احتياطي"),this.currentOptions=this.generateOptions(this.currentCorrectText,e));else if(this.examType==="sprach1")this.currentOptions=e.options||[],this.currentOptions.length===0&&(console.warn("⚠️ لا توجد خيارات في السؤال، نستخدم generateOptions كحل احتياطي"),this.currentOptions=this.generateOptions(this.currentCorrectText,e));else if(this.examType==="sprach2"){const o=e.options||[],r=e.connector||e.correct,a=o.filter(l=>l!==r).sort(()=>Math.random()-.5).slice(0,2);this.currentOptions=this.shuffleArray([r,...a])}else this.currentOptions=this.generateOptions(this.currentCorrectText,e);let i="",n="";if(this.examType==="matching")i=this.currentSkill==="lesen3"?"اختر الحالة المناسبة للفقرة التي قرأتها:":"اختر العنوان المناسب للنص الذي قرأته:";else if(this.examType==="multiple")i="ما الاختيار الصحيح؟",n=`
            ...
            `;else if(this.examType==="sprach1"){i="اختر الكلمة الصحيحة:";const o=e.memoryHighlight||{},r=e.id||this.currentQuestionIndex+1,a=o.before||"",l=o.after||"";n=`
                <div style="
                    font-size: 18px;
                    font-weight: 500;
                    text-align: left;
            ...
            `}else if(this.examType==="sprach2"){i="اختر الكلمة الصحيحة:";const o=e.memoryHighlight||{},r=e.id||this.currentQuestionIndex+1,a=o.before||"",l=o.after||"";n=`
                <div style="
                    font-size: 18px;
                    font-weight: 500;
                    text-align: left;
...
`).join("")}
        ...
        `)}checkAnswer(e){if(this.isAnswered)return;this.isAnswered=!0,this.attempts++;const t=this.currentOptions[e];let i=!1,n="";if(this.examType==="matching"){let d=null;if(this.currentQuestionObj._sharedOptions&&this.currentQuestionObj._sharedOptions.length>0?d=this.currentQuestionObj._sharedOptions:this.currentQuestionObj.examId&&this.examSharedOptionsMap[this.currentQuestionObj.examId]?d=this.examSharedOptionsMap[this.currentQuestionObj.examId]:this.sharedOptions&&this.sharedOptions.length>0&&(d=this.sharedOptions),d){const c=this.currentCorrectIndex,h=d[c];i=t===h,n=h}else{const c=this.currentQuestionObj.correct;n=this.sharedOptions[c]||"",i=t===n}}else if(this.examType==="multiple"){const d=this.currentQuestionObj.correct,c=this.currentOptions[d];i=t===c,n=c}else if(this.examType==="sprach1"){const d=this.currentQuestionObj.connector||this.currentQuestionObj.correct;i=t===d,n=d}else if(this.examType==="sprach2"){const d=this.currentQuestionObj.connector||this.currentQuestionObj.correct;i=t===d,n=d}else i=t===this.currentCorrectText,n=this.currentCorrectText;const s=this.currentSkill,o=this.currentExamId,r=this.currentQuestionIndex,a=this.buildSentenceId(s,o,r),l=document.querySelectorAll(".memory-trainer-option"),p=document.getElementById("memory-trainer-feedback");l.forEach(d=>{d.disabled=!0,d.style.opacity="0.7",d.style.cursor="default"}),i?(this.correctAttempts++,this.increaseLevel(a),l[e].style.borderColor="#28a745",l[e].style.backgroundColor="#d4edda",p.innerHTML='<button class="memory-trainer-btn primary small" onclick="window.memoryTrainer.nextQuestion()" style="padding:6px 16px; border:none; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; background:#28a745; color:white;">التالي →</button>'):(this.decreaseLevel(a),this.wrongQuestions.includes(this.currentQuestionObj)||this.wrongQuestions.push(this.currentQuestionObj),l[e].style.borderColor="#e67e22",l[e].style.backgroundColor="#fef0e0",l.forEach((d,c)=>{this.currentOptions[c]===n&&(d.style.borderColor="#28a745",d.style.backgroundColor="#d4edda")}),p.innerHTML=`
            ...
            `),this.isFromList&&this.updateProgressBar()}updateProgressBar(){const e=this.getOverallProgressForSkill(this.currentSkill),t=this.card?.querySelectorAll(".memory-progress-fill, .memory-trainer-progress-bar");t&&t.forEach(n=>{n.classList.contains("memory-progress-fill")&&(n.style.width=e+"%")});const i=this.card?.querySelector(".memory-progress-percent");i&&(i.textContent=e+"%")}retryQuestion(){this.isAnswered=!1,this.currentIndex--,this.nextQuestion()}nextQuestion(){this.currentIndex++,this.currentIndex<this.trainingQueue.length?this.showMemoryCard():this.showPhaseComplete()}showPhaseComplete(){this.clearTimer();const e=this.wrongQuestions.length,t=this.isFromList?this.getOverallProgressForSkill(this.currentSkill):0;if(e===0){this.showResults();return}this.updateCard(`
        ...
        `)}startReview(){this.isReviewMode=!0,this.trainingQueue=[...this.wrongQuestions],this.currentIndex=0,this.totalQuestions=this.trainingQueue.length,this.wrongQuestions=[],this.showMemoryCard()}showResults(){const e=this.isFromList,t=this.currentSkill,i=this.currentExamId||1,n=this.getExamProgress(t,i),s=this.getOverallProgressForSkill(t),o=this.getStageProgressForSkill(t);let r="";if(e){let a=1,l=1,p=!1;if(window.getCurrentStage&&window.getTotalStages)a=window.getCurrentStage(t),l=window.getTotalStages(t),p=a>=l;else{const h=`_${t}_combinedData`;if(window[h]){const x=window[h];a=x.currentStage||1,l=x.totalStages||1,p=x.isLastStage||a>=l}}let d=this.totalQuestions||0;const c=`_${t}_combinedData`;window[c]&&(d=window[c].totalQuestions||d),p?r=`
