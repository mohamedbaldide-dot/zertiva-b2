...
function createHelpBoxWithContent(index, totalQuestions) {
    ...
    const helpContent = (window.HELP_DATA && window.HELP_DATA[helpKey]) ? window.HELP_DATA[helpKey] : null;
...
function createHelpBoxesWithContent(count, correctQuestions) {
    ...
    if (count === 10) {
        ...
        for (let row = 0; row < 5; row++) {
            ...
            for (let col = 0; col < 2; col++) {
                ...
                if (validIndices.includes(index)) {
                    ...
                    emptyDiv.style.cssText = 'display: none;';
        ...
        for (let i = 0; i < 5; i++) {
            const index = i + 1;
            if (validIndices.includes(index)) {
                column.appendChild(createHelpBoxWithContent(index, count));
            }
        }
...
function toggleHelpLayer() {
    ...
    if (helpLayerActive) {
        ...
        originalContentBackup = { hiddenQuestions: hiddenQuestions, hiddenButtons: hiddenButtons };
...
function addHelpButtonToExam() {
    ...
    if (navButtons && !document.getElementById('globalHelpButton')) {
