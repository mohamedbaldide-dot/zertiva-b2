...
// ============================================
// helpSystem.js - نظام المساعدة المتكامل (النسخة النهائية المصححة)
// ============================================
...
// ============================================
// دالة البحث عن البيانات بكل الصيغ الممكنة
// ============================================
function findHelpData(skill, examId, questionNumber) {
    ...
    const possibleKeys = [
        ...
        // صيغ إضافية احتياطية
        `${skill}_exam${examId}_question_${questionNumber}`,
        `${skill}_exam${examId}_frage_${questionNumber}`,
    ];
    
    // تجربة كل الصيغ
    for (let key of possibleKeys) {
        if (HELP_DATA[key]) {
            console.log(`✅ وجدت البيانات بالمفتاح: ${key}`);
            return HELP_DATA[key];
        }
    }
    
    // محاولة البحث المباشر في HELP_DATA بكل المفاتيح (فحص شامل)
    for (let key in HELP_DATA) {
        if (key.includes(`exam${examId}`) && key.includes(`${questionNumber}`)) {
            console.log(`✅ وجدت البيانات بالمفتاح (فحص شامل): ${key}`);
            return HELP_DATA[key];
        }
        // للأسئلة بدون 'q' ومع شرطة سفلية
        if (key.includes(`exam${examId}`) && key.includes(`_${questionNumber}`) && !key.includes('_q')) {
            console.log(`✅ وجدت البيانات بالمفتاح (فحص شامل - بدون q): ${key}`);
            return HELP_DATA[key];
        }
    }
...
function getCurrentSkill() {
    ...
    // محاولة بديلة: البحث داخل الزر النشط
    const activeTeil = document.querySelector('.teil-btn.active');
    if (activeTeil) {
        const teilId = activeTeil.getAttribute('data-teil');
        if (teilId) {
            console.log("القسم من الزر:", teilId);
            return teilId;
        }
    }
    
    // محاولة بديلة: البحث من عنوان الامتحان
    const title = document.getElementById('examTitle')?.textContent || '';
    if (title.includes('Hören 1') || title.includes('hoeren1')) return 'hoeren1';
    if (title.includes('Hören 2') || title.includes('hoeren2')) return 'hoeren2';
    if (title.includes('Hören 3') || title.includes('hoeren3')) return 'hoeren3';
    if (title.includes('Lesen 1') || title.includes('lesen1')) return 'lesen1';
    if (title.includes('Lesen 2') || title.includes('lesen2')) return 'lesen2';
    if (title.includes('Lesen 3') || title.includes('lesen3')) return 'lesen3';
    if (title.includes('Sprachbausteine 1') || title.includes('sprach1')) return 'sprach1';
    if (title.includes('Sprachbausteine 2') || title.includes('sprach2')) return 'sprach2';
    
    console.log("لم يتم تحديد القسم، استخدام القيمة الافتراضية: hoeren1");
    return 'hoeren1';
}
...
function getCurrentExamId() {
    if (window.currentExamId && window.currentExamId > 0) {
        console.log("Exam ID من window.currentExamId:", window.currentExamId);
    ...
    if (match) {
        console.log("Exam ID من title:", match[1]);
    ...
    if (window.location.search) {
        ...
        if (match) {
            console.log("Exam ID من URL:", match[1]);
            return parseInt(match[1]);
        }
    }
    ...
    if (match) {
        console.log("Exam ID من body:", match[1]);
        return parseInt(match[1]);
    }
    
    console.log("لم يتم تحديد Exam ID، استخدام القيمة الافتراضية: 1");
    return 1;
}

// ============================================
// دالة الحصول على الأسئلة الصحيحة لكل امتحان
// ============================================
function getCorrectQuestions(skill, examId) {
    ...
    console.log("البحث عن الأسئلة الصحيحة للمفتاح:", key);
    const result = correctMap[key] || [];
    console.log("الأسئلة الصحيحة:", result);
    return result;
}

// ============================================
// دالة إنشاء بطاقة المساعدة
// ============================================
function createHelpCard(questionNumber) {
    const examId = getCurrentExamId();
    const skill = getCurrentSkill();
    
    console.log(`🔍 إنشاء بطاقة للسؤال ${questionNumber} - القسم: ${skill} - الامتحان: ${examId}`);
    
    // استخدام دالة البحث الشاملة
    ...
    if (data && data.text) {
        ...
        card.innerHTML = `
            <div style="font-weight:bold;color:#2c3e66;border-right:4px solid #007bff;padding-right:12px;margin-bottom:15px;font-size:17px">
                ${questionNumber}️⃣ ${data.text.substring(0, 150)}${data.text.length > 150 ? '...' : ''}
            </div>
...
// ============================================
// دالة إنشاء طبقة المساعدة الكاملة
// ============================================
function createHelpLayer() {
    ...
    console.log(`إنشاء طبقة مساعدة لـ ${correctQuestions.length} سؤال`);
...
// ============================================
// دوال التحكم في إظهار/إخفاء المحتوى
// ============================================
...
// ============================================
// دالة تبديل وضع المساعدة الرئيسية
// ============================================
function toggleHelp() {
    const existing = document.getElementById('helpLayerContainer');
    const section = getActiveSection();
    
    console.log("تبديل وضع المساعدة - الحالة الحالية:", helpLayerActive);
    ...
    if (helpLayerActive) {
        ...
        console.log("تم إيقاف وضع المساعدة");
    } else {
        ...
        console.log("تم تشغيل وضع المساعدة");
    }
}
...
function addHelpButton() {
    ...
    nav.appendChild(btn);
    console.log("تم إضافة زر المساعدة");
}

// ============================================
// مراقبة التغييرات وإضافة الزر
// ============================================
...
console.log('✅ helpSystem.js تم التحميل بنجاح (النسخة النهائية)');